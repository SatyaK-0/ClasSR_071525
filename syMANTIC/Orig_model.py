...
class SymanticModel:
  ...
  def __init__(self,df,operators=None,multi_task = None,n_expansion=None,n_term=None,sis_features=20,device='cpu',relational_units = None,initial_screening = None,dimensionality=None,output_dim = None,metrics=[0.06,0.995],disp=False,pareto=False, A=None, B = None):
    ...
    #CODE TO ENABLE SAVES:
    self.save_progress = save_progress
    self.progress_saver = ProgressSaver(progress_path) if save_progress else None
  ...
  def fit(self, A = None, B = None):
    ...
    if self.save_progress:
            ...
            progress = ProgressSaver.load_latest_progress(self.progress_saver.save_path)
            if progress:
...
def save_model(self, filepath):
    ...
    Args:
        filepath (str): Path to save the model (should end with .pkl or .joblib)
    """
    import pickle
    import joblib
    ...
    try:
        # Try using joblib first (more efficient for large numpy arrays)
...
def load_model(cls, filepath, df=None):
    ...
    Args:
        ...
        df (pd.DataFrame): Optional - provide if you want to use the model with new data
        
    Returns:
        SymanticModel: Loaded model instance
    """
    import pickle
    import joblib
    ...
    if df is None:
        # If no DataFrame provided, use the one from training if available
        if 'df' in model_data:
            df = model_data['df']
        else:
    ...
    model = cls(
        ...
        n_expansion=None,  # Will be set from final_df
...
def get_best_equation(self):
    ...
    Returns:
        dict: Dictionary containing the best equation and its metrics
    """
    if self.final_df is None:
        raise ValueError("Model has not been trained yet")
        
    # Find the row with minimum loss
