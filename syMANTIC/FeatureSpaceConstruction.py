...
def batched_indices(total, batch_size):
    for i in range(0, total, batch_size):
        yield slice(i, min(i+batch_size, total))
...
def pad_tensors_for_cat(t1, t2):
    # Pads the smaller in columns to match the bigger, for concatenation along rows.
    if t1.shape[1] < t2.shape[1]:
        pad = torch.full((t1.size(0), t2.shape[1] - t1.shape[1]), float('nan'), device=t1.device)
        t1 = torch.cat([t1, pad], dim=1)
    elif t2.shape[1] < t1.shape[1]:
        pad = torch.full((t2.size(0), t1.shape[1] - t2.shape[1]), float('nan'), device=t2.device)
...
class feature_space_construction:
  ...
  def batched_regressor_fit(self, X, y, columns, complexity, dimension, sis_features, device, metrics, batch_size=100, A=None, B=None, disp=False):
    """
    Fit regressor in batches over features (columns). Useful for high-dimensional spaces.
    Returns concatenated results across all batches.
    """
    from syMANTIC.Regressor import Regressor

    n_features = X.shape[1]
    all_rmse, all_eqn, all_r2, all_c, all_n, all_intercepts, all_coeffs, all_r2_value = [], [], [], [], [], [], [], []
    for batch_slice in batched_indices(n_features, batch_size):
        X_batch = X[:, batch_slice]
        cols_batch = [columns[j] for j in range(batch_slice.start, batch_slice.stop)]
        comp_batch = complexity[batch_slice]

        reg = Regressor(
            X_batch, y, cols_batch, comp_batch, dimension, sis_features, device, metrics=metrics, A=A, B=B, disp=disp
        )
        try:
            rmse, eqn, r2, r, c, n, intercepts, coeffs, r2_value = reg.regressor_fit()
        except Exception as e:
            print(f"Batch {batch_slice} failed: {e}")
            continue
        ...
        all_r2_value.append(r2_value)
        del X_batch, cols_batch, comp_batch, reg
  ...
  def single_variable(self, operators_set, i, batch_size=100):
    ...
    for op in operators_set:
        ...
        old_ref, new_ref = pad_tensors_for_cat(self.reference_tensor, feature_values_reference)
        self.reference_tensor = torch.cat((old_ref, new_ref), dim=0)
        ...
        if operators_reference.dim() == 1:
            operators_reference = operators_reference.unsqueeze(1)
        old_op, new_op = pad_tensors_for_cat(self.operators_final, operators_reference)
        del feature_values_reference, operators_reference
        self.operators_final = torch.cat((old_op, new_op), dim=0)
        self.operators_final = self.clean_tensor(self.operators_final)
  ...
  def combinations(self, operators_set, i, batch_size=1500):
    ...
    if reference_rows:
        new_refs = torch.cat(reference_rows, dim=0)
        old_ref, new_ref = pad_tensors_for_cat(self.reference_tensor, new_refs)
        self.reference_tensor = torch.cat((old_ref, new_ref), dim=0)
        self.reference_tensor = self.clean_tensor(self.reference_tensor)
        del reference_rows
    if operators_rows:
        ...
        if self.operators_final.dim() == 1:
            self.operators_final = self.operators_final.unsqueeze(1)
        if new_ops.dim() == 1:
            new_ops = new_ops.unsqueeze(1)
        old_op, new_op = pad_tensors_for_cat(self.operators_final, new_ops)
        self.operators_final = torch.cat((old_op, new_op), dim=0)
        self.operators_final = self.clean_tensor(self.operators_final)
        del operators_rows
  ...
  def feature_space(self):
    ...
    if self.no_of_operators == None:
        ...
        rmse1, equation1,r21,r,c,n,intercepts,coeffs,r2_value = self.batched_regressor_fit(self.df_feature_values,self.Target_column,self.columns,complexity,self.dimension,self.sis_features,self.device,metrics = self.metrics, A=self.A, B=self.B, batch_size = 500)
        ...
        while True:
            ...
            rmse, equation,r2,r,c,n,intercepts,coeffs,r2_value = self.batched_regressor_fit(self.df_feature_values,self.Target_column,self.columns,complexity,self.dimension,self.sis_features,self.device,metrics = self.metrics, A=self.A, B = self.B, batch_size = 500)
